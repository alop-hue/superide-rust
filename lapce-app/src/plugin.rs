...
const MARKETPLACE_DEFAULT: &str = "https://plugins.lapce.dev";

fn marketplace_url() -> String {
    std::env::var("SUPERIDE_MARKETPLACE")
        .unwrap_or_else(|_| MARKETPLACE_DEFAULT.to_string())
}
...
impl PluginData {
    ...
    pub fn volt_installed(&self, volt: &VoltMetadata, icon: &Option<Vec<u8>>) {
        ...
        if !is_latest {
        let base = marketplace_url();
        let url = format!(
            "{base}/api/v1/plugins/{}/{}/latest",
    ...
    fn download_readme(
        ...
        let base = marketplace_url();
        let url = format!(
            "{base}/api/v1/plugins/{}/{}/{}/readme",
            volt.author, volt.name, volt.version
        );
        let resp = match lapce_proxy::get_url(&url, None) {
            Ok(r) => r,
            Err(e) => {
                let text = parse_markdown(
                    &format!("Marketplace unreachable: {e}"),
                    2.0, config,
                );
                return Ok(text);
            }
    ...
    fn query_volts(query: &str, offset: usize) -> Result<VoltsInfo> {
        let base = marketplace_url();
        let url = format!("{base}/api/v1/plugins?q={query}&offset={offset}");
        let resp = lapce_proxy::get_url(url, None)?;
        let plugins: VoltsInfo = resp.json()?;
        Ok(plugins)
    }
