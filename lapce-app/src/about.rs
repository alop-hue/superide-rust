...
use floem::{
    ...
    views::{Decorators, container, img, label, stack},
};
...
const SUPER_IDE_MARK_PNG: &[u8] =
    include_bytes!("../../extra/images/superide/superide-mark.png");

struct AboutUri {}

impl AboutUri {
    const WEBSITE: &'static str = "https://superide.dev";
    const GITHUB: &'static str = "https://github.com/super-ide/super-ide";
...
pub fn about_popup(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    exclusive_popup(window_tab_data, about_data.visible, move || {
        stack((
            img(move || SUPER_IDE_MARK_PNG.to_vec()).style(move |s| {
                s.size(logo_size, logo_size)
            }),
            label(|| "SUPER IDE".to_string()).style(move |s| {
                s.font_bold()
                    .margin_top(10.0)
            ...
            label(|| "Built on Lapce".to_string()).style(move |s| {
                s.margin_top(2.0)
                    .font_size(11.0)
                    .color(config.get().color(LapceColor::EDITOR_DIM))
            }),
            ...
            label(|| "Built On".to_string()).style(move |s| {
                s.font_bold()
                    .color(config.get().color(LapceColor::EDITOR_DIM))
                    .margin_top(40.0)
            }),
            web_link(
                || "Lapce Editor".to_string(),
