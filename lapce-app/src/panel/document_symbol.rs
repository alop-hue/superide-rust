        ...
        virtual_stack(
            {
                ...
                move || {
                    ...
                    // Track both the editor signal and the inner document_symbol_data signal
                    // so that changes to either trigger a re-render
                    let sym_data = editor.and_then(|e| {
                        let ds = e.doc().document_symbol_data;
                        ds.get();
                        Some(ds)
                    });
                    VirtualList::new(sym_data)
                }
            },
