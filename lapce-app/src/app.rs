...
#[derive(Debug)]
struct Cli {
    /// Launch new window even if SUPER IDE is already running
...
fn window_tab(window_tab_data: Rc<WindowTabData>) -> impl View {
    ...
    let view = stack((
        ...
        welcome::welcome_view(window_tab_data.clone()),
    ))
    .on_cleanup(move || {
...
fn window(window_data: WindowData) -> impl View {
    ...
    .window_title(move || {
        ...
        match workspace {
            Some(workspace) => format!("{workspace} - SUPER IDE"),
            None => "SUPER IDE".to_string(),
        }
    })
...
pub fn launch() {
    ...
    trace!(TraceLevel::INFO, "Starting up SUPER IDE..");
    ...
    if !cli.wait {
        ...
        if let Err(why) = cmd
            ...
            .spawn()
        {
            eprintln!("Failed to launch SUPER IDE: {why}");
            std::process::exit(1);
        };
        return;
    }
    ...
    let db = match LapceDb::new() {
        ...
        Err(e) => {
            ...
            logging::error_modal(
                "Error",
                &format!("Failed to create SUPER IDE database: {e}"),
            );

            trace!(
                TraceLevel::ERROR,
                "Failed to create SUPER IDE database: {e}"
            );
            std::process::exit(1);
        }
    };
...
pub fn window_menu(
    ...
    Menu::new("SUPER IDE")
        .entry({
            let mut menu = Menu::new("SUPER IDE")
                .entry(MenuItem::new("About SUPER IDE").action(move || {
                ...
                .separator()
                .entry(MenuItem::new("Quit SUPER IDE").action(move || {
                    workbench_command.send(LapceWorkbenchCommand::Quit);
                }));
            if cfg!(target_os = "macos") {
                menu = menu
                    .separator()
                    .entry(MenuItem::new("Hide SUPER IDE"))
